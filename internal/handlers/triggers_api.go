...
func (h *PubSubHandler) PubSubPushGeminiHandler(w http.ResponseWriter, r *http.Request) {
	...
	data, err := extractBudgetNumbers(decoded)
	if err != nil || data == nil {
		log.Printf("warning: failed to extract budget numbers: %v", err)
		w.WriteHeader(http.StatusOK)
		return
	}

	// If this is a Cloud Billing budget message, ignore until 90% threshold
	if shouldIgnoreByBudget(data.CostAmount, data.BudgetAmount) {
		w.WriteHeader(http.StatusOK)
		return
	}
	_, err = h.db.CreateAPITrigger(r.Context(), "gemini", data.BudgetAmount, data.CostAmount)
	if err != nil {
...
func (h *PubSubHandler) PubSubPushAudioHandler(w http.ResponseWriter, r *http.Request) {
	...
	_, err = h.db.CreateAPIAudioTrigger(r.Context(), "audio", &database.APITriggerOptions{BudgetAmount: data.BudgetAmount, CostAmount: data.CostAmount, Threshold: data.Threshold, DisplayName: data.DisplayName, Currency: data.Currency})
	if err != nil {
		http.Error(w, "failed to create api audio trigger", http.StatusInternalServerError)
		return
	}
	w.WriteHeader(http.StatusOK)
}
...
func extractBudgetNumbers(s []byte) (*PubSubData, error) {
	var data PubSubData
